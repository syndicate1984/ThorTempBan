...
CREATE TABLE IF NOT EXISTS vrp_users(
  ...
  username TEXT NOT NULL DEFAULT 'Player Name',
  last_login VARCHAR(100),
  whitelisted BOOLEAN NOT NULL DEFAULT '0',
  banned BOOLEAN NOT NULL DEFAULT '0',
  tempban VARCHAR(100) NOT NULL DEFAULT 'No Temp Ban',
  bannedReason VARCHAR(120) NOT NULL DEFAULT 'No Ban Reason',
  bannedBy TEXT NOT NULL DEFAULT 'No Ban By',
  ...
  if ids ~= nil and #ids > 0 then
    ...
    vRP.getUserIdByIdentifiers(ids, function(user_id)
      ...
      if user_id ~= nil then -- check user validity 
        ...
        Wait(2000)
        deferrals.update("[vRP] Checking Tempbanned...")
        vRP.isBanned(user_id, function(banned)
          vRP.getBannedTime(user_id,function(banDate)
            ...
            if not banned and (tostring(actDate) == tostring(banHash[2])) and (tostring(actDate) <= tostring(banHash[2]))  then
              ...
              vRP.getBannedReason(user_id, function(ban)
                banReason = ban.banReason or ""
                bannedBy = ban.bannedBy or ""
                print("[vRP] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: banned (user_id = "..user_id..")")
                deferrals.done("[vRP] Esti banat pe Thor Vintage! \nBanat De: "..bannedBy.."\nMotiv: "..banReason.."\nID-ul Tau: ["..user_id.."]\n\nPentru unban appeal intra pe Discord: discord.io/thorvintage")
              end)
            end
          end)
        end)
